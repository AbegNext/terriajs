...
var ArcGisMapServerCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the the smallest scale for which tiles should be requested.  For example, if this value is 1000, then tiles representing
     * a scale larger than 1:1000 will not be requested.  Instead, tiles of the smallest-available scale, as specified by this property,
    ...
    /**
     * Gets or sets the largest scale for which tiles will show data.  There is nothing reasonable we can do to fill
     * in the data at higher levels, so this is detected in order to possibly provide a warning to the user.
     * @type {Number}
     */
    this.minimumScale = undefined;

    knockout.track(this, ['url', 'layers', 'maximumScale', 'minimumScale', '_legendUrl']);
...
ArcGisMapServerCatalogItem.prototype._createImageryProvider = function() {
    ...
    if (defined(this.minimumScale) && this.minimumScale < 25000000) {
